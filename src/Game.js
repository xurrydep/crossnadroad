...
class Chicken{
    constructor(size = {x: 0.63, y: 0.6, z: 0.63}){
        ...
        let red = new THREE.MeshLambertMaterial({color: 0xdc5a5a}),
            white = new THREE.MeshLambertMaterial({color: 0x8a2be2}),
            orange = new THREE.MeshLambertMaterial({color: 0xda6400}),
            black = new THREE.MeshLambertMaterial({color: 0x000000});

        let foot1 = new THREE.BoxGeometry(0.1, 0.1, 0.6),
            foot2 = new THREE.BoxGeometry(0.1, 0.1, 0.3),
        ...
        let lowerBody = new THREE.BoxGeometry(0.69, 0.6, 1.2),
            upperBody = new THREE.BoxGeometry(0.69, 0.675, 0.8),
        ...
        if (!this.isMoving && !gameOver){
            ...
            switch (direction) {
                case 'left':
                    ...
                    this.currentColumn--;
                    dX = -cellWidth;
                    this.model.rotation.y = THREE.MathUtils.degToRad(-90);
                    break;
                case 'up':
            ...
            if(this.currentLane > this.maxLane){
                ...
                scene.add(lane.mesh);
                const scoreEl = document.getElementById("score");
        if (scoreEl) scoreEl.innerText = "SCORE:" + this.maxLane;
            }
...
class Rail{
    constructor(){
        ...
        let leftRailGeo1 = new THREE.BoxGeometry(cellWidth * columns, cellWidth/20, cellWidth/10),
            leftRailGeo2 = new THREE.BoxGeometry(cellWidth * columns, cellWidth/20, cellWidth/10),
            centerRailGeo1 = new THREE.BoxGeometry(cellWidth * columns, cellWidth/20, cellWidth/10),
        ...
        let sideRails = new THREE.Mesh(BufferGeometryUtils.mergeGeometries(sideBars), new THREE.MeshPhongMaterial({color: 0x7b3c2a}));
        this.model.add(sideRails);
        let centerRails = new THREE.Mesh(BufferGeometryUtils.mergeGeometries(centerBars), new THREE.MeshPhongMaterial({color: 0x9D5E4C}));
...
class Car{
    constructor(color, size = {x: 0.95, y: 0.9, z: 0.8}){
        ...
        let lowerBody = new THREE.Mesh(new THREE.BoxGeometry(4, 0.75, 1.8), new THREE.MeshLambertMaterial({color: light}));
        ...
        let wheel1 = new THREE.BoxGeometry(2 * lowerBody.geometry.parameters.height/3, 2 * lowerBody.geometry.parameters.height/3, 1.85),
            wheel2 = new THREE.BoxGeometry(2 * lowerBody.geometry.parameters.height/3, 2 * lowerBody.geometry.parameters.height/3, 1.85),
        ...
        let strip1 = new THREE.BoxGeometry(4.005, 0.8, 1),
        ...
        let strip5 = new THREE.Mesh(new THREE.BoxGeometry(4.01, 0.25, 1.81), new THREE.MeshLambertMaterial({color: 0x645991}));
        ...
        // Add "2025" text on the roof of the car
        let carCanvas = document.createElement('canvas');
        carCanvas.width = 128;
        carCanvas.height = 32;
        let carContext = carCanvas.getContext('2d');
        carContext.clearRect(0, 0, carCanvas.width, carCanvas.height);
        carContext.fillStyle = '#000000'; // Black text
        carContext.font = 'bold 27px Arial';
        carContext.textAlign = 'center';
        carContext.textBaseline = 'middle';
        carContext.fillText('2025', carCanvas.width / 2, carCanvas.height / 2);
        
        let carTextTexture = new THREE.CanvasTexture(carCanvas);
        let carTextMaterial = new THREE.MeshLambertMaterial({ map: carTextTexture, transparent: true });
        let carTextGeometry = new THREE.PlaneGeometry(1.5, 0.4);
        let carTextMesh = new THREE.Mesh(carTextGeometry, carTextMaterial);
        carTextMesh.position.set(1.5, 2.0, 0); // Positioned at X=1.5, Y=2.0, Z=0
        carTextMesh.rotation.x = -Math.PI / 2; // Rotate to face upward
        this.model.add(carTextMesh);
...
class Truck{
    constructor(color, size = {x: 0.95, y: 1, z: 1}){
        ...
        let middleStrip = new THREE.Mesh(new THREE.BoxGeometry(1.61, 0.5, 1.41), new THREE.MeshLambertMaterial({color: dark}));
        ...
        let window = new THREE.BoxGeometry(0.85, 0.5, 1.45),
        ...
        // Add PNG logo above MAINNET text
        let logoLoader = new THREE.TextureLoader();
        logoLoader.load('assets/images/monad.png', (logoTexture) => {
            let logoMaterial = new THREE.MeshLambertMaterial({ map: logoTexture, transparent: true });
            let logoGeometry = new THREE.PlaneGeometry(0.8, 0.8);
            let logoMesh = new THREE.Mesh(logoGeometry, logoMaterial);
            logoMesh.position.set(2.7, 1.8, 1.05); // Above the text
            logoMesh.rotation.y = 0;
            this.model.add(logoMesh);
            
            // Add logo to the other side too
            let logoMesh2 = logoMesh.clone();
            logoMesh2.position.set(2.7, 1.8, -1.05);
            logoMesh2.rotation.y = Math.PI;
            this.model.add(logoMesh2);
        });

        // Create MAINNET text on the side of the truck
        ...
        context.fillText('MAINNET', canvas.width / 2, canvas.height / 2);
        
        let textTexture = new THREE.CanvasTexture(canvas);
        let textMaterial = new THREE.MeshLambertMaterial({ map: textTexture, transparent: true });
        let textGeometry = new THREE.PlaneGeometry(2.2, 0.55);
        let textMesh = new THREE.Mesh(textGeometry, textMaterial);
        textMesh.position.set(2.7, 1.2, 1.05); // Position moved to X=2.7, Z=1.05
        textMesh.rotation.y = 0; // Face forward
        this.model.add(textMesh);
        
        // Add text to the other side too
        let textMesh2 = textMesh.clone();
        textMesh2.position.set(2.7, 1.2, -1.05); // Position moved to X=2.7, Z=-1.05 on the other side
        textMesh2.rotation.y = Math.PI; // Face the other direction
        this.model.add(textMesh2);
    ...
    constructor(length, size = {x: 1, y: 1, z: 0.95}){
        ...
        meshes.forEach(mesh =>{
            if (mesh.geometries.length){
                let merged = new THREE.Mesh(BufferGeometryUtils.mergeGeometries(mesh.geometries), new THREE.MeshLambertMaterial({color: mesh.color}));
...
class Tree{
    constructor(layers, size = {x: 0.95, y: 1, z: 0.95}){
        ...
        this.model = new THREE.Group();
        let trunk = new THREE.Mesh(new THREE.BoxGeometry(width/2, height, width/2), new THREE.MeshLambertMaterial({color: 0x91654B}));
        ...
        let allBrightLeaves = new THREE.Mesh(BufferGeometryUtils.mergeGeometries(brightLeaves), new THREE.MeshLambertMaterial({color: 0xabdf00}));
...
class Log{
    constructor(size = {x: 0.95, y: 1, z: 0.8}){
        ...
        this.model.add(new THREE.Mesh(BufferGeometryUtils.mergeGeometries(lightMeshes), new THREE.MeshLambertMaterial({color: 0x8d5358})));

        let middle1 = new THREE.BoxGeometry(0.2, 0.25, 1.2),
            front1 = new THREE.BoxGeometry(0.2, 0.5, 0.2),
            back1 = new THREE.BoxGeometry(0.2, 0.5, 0.2),
        ...
        this.model.add(new THREE.Mesh(BufferGeometryUtils.mergeGeometries(whiteMeshes), new THREE.MeshLambertMaterial({color: 0xefb9be})));
        ...
        this.model.add(new THREE.Mesh(BufferGeometryUtils.mergeGeometries(darkMeshes), new THREE.MeshLambertMaterial({color: 0x491025})));
...
class Feathers{
    constructor(range = 2, height = 6.3){
        ...
        this.coordinates.forEach(() =>{
            ...
            let mat = new THREE.MeshLambertMaterial({color: 0x8a2be2, side: THREE.DoubleSide});
    ...
    animate(initialPos, duration = 5){
        this.feathers.forEach((feather, index) =>{
            ...
            this.animations[index] = {
                ...
                clock: new THREE.Clock(),
                rotationSpeed: THREE.MathUtils.degToRad(Math.random() * 360),
...
class Splash{
    constructor(range = 2, height = 5){
        ...
        this.coordinates.forEach(() =>{
            ...
            let mat = new THREE.MeshLambertMaterial({color: 0x9932cc});
...
class Sound{
    constructor(listenerParent){
        ...
        const pressPlayEl = document.getElementById("pressPlay");
        if (pressPlayEl) pressPlayEl.disabled = false;
        });
...
// Submit score to both smart contract and leaderboard API
const submitScore = async (score) => {
    try {
        ...
        try {
            ...
            console.error('Error submitting score to smart contract:', contractError);
        }
        ...
        try {
            ...
            console.error('Error submitting score to leaderboard API:', apiError);
        }
